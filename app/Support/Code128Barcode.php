...
class Code128Barcode
{
    ...
    public function dataUri(string $value, int $height = 58): string
    {
        $svg = $this->svg($value, $height);
    ...
    public function svg(string $value, int $height = 58): string
    {
        ...
        return sprintf(
            '<svg xmlns="http://www.w3.org/2000/svg" width="%d" height="%d" viewBox="0 0 %d %d"><g fill="#111827">%s</g></svg>',
            $x,
            $height,
            $x,
            $height,
    ...
    private function codes(string $value): array
    {
        ...
        foreach (str_split($value) as $position => $character) {
            ...
            if ($ascii < 32 || $ascii > 126) {
                throw new \InvalidArgumentException('Code 128 set B supports ASCII characters between 32 and 126.');
            }
